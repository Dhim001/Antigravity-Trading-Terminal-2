...
class OrderManager:
    ...
    def get_account_data(self):
        ...
        # Get positions
        cursor.execute("SELECT symbol, size, avg_price FROM positions WHERE size != 0.0")
        positions = {row["symbol"]: {"size": row["size"], "avg_price": row["avg_price"]} for row in cursor.fetchall()}
    ...
    def place_order(self, symbol, order_type, side, price, quantity):
        ...
        try:
            ...
            if order_type == "MARKET":
                self._process_fill(cursor, symbol, side, market_price, quantity, quote)
    ...
    def match_pending_orders(self):
        ...
        cursor.execute("SELECT id, symbol, side, price, quantity FROM orders WHERE status = 'PENDING'")
    ...
    def _process_fill(self, cursor, symbol, side, price, quantity, quote):
        ...
        if not pos_row:
            ...
            cursor.execute("""
                INSERT INTO positions (symbol, size, avg_price)
                VALUES (?, ?, ?)
            """, (symbol, new_size, price))
        else:
            ...
            # If position is flat, clear average price
