...
- Train/validate are **async jobs** (`POST /api/v1/ml/train|validate` + `GET /api/v1/ml/jobs/{id}`) with
  progress files and cancel; client timeouts are long (HistGBM validate ≈ 90 min).
- Panel progress uses **server job progress** when available (fallback estimate only if missing).
- Jobs run in a `ProcessPoolExecutor` (`ML_TRAIN_MAX_WORKERS`); queue/cancel via `/ml/jobs`.
- `GET /ml/model-status` (`_ml_status_enrich`) includes `validated_at` / `walk_forward` / `pbo`,
  `version_id`, and `champion_desynced` when the live root lags the newest snapshot.
- Strategy / subdir / trainer maps are centralized in `backend/app/services/bots/ml_registry.py`
  (Lab artifact strategies). Meta-label models remain bot-scoped under `meta_label_models/`.
- `MlRetrainScheduler.get_retrain_history()` / `get_pending()` exist; retrain queue exposed via
  `/ml/retrain-status`.
- Feature importance / confusion matrix live in Backtest Lab (`MlOptimizerPanel`) and increasingly
  in the training dock dataset browser.
...
- In-memory dict of jobs: `{job_id, kind: train|validate, strategy, symbol, status: queued|running|done|error|cancelled, started_at, finished_at, progress: {pct, phase, detail}, result_ref}`.
