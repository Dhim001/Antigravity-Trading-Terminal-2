/**
 * TA parameter sweep + walk-forward controls with strategy-aware param grid.
 */
...
function metricHeader(objective) {
  ...
  if (objective === 'calmar_ratio') return 'Calmar';
  if (objective === 'robust_score') return 'Robust';
  if (objective === 'auc_roc') return 'AUC';
  if (objective === 'log_loss') return '−LogLoss';
  if (objective === 'alpha_decay_half_life') return 'Half-life';
  if (objective === 'oos_is_ratio') return 'OOS/IS';
  return 'PnL';
}

function formatMetric(row, objective) {
  const summary = row.summary ?? {};
  const ml = row.ml_metrics ?? {};
  ...
  if (objective === 'alpha_decay_half_life') {
    const v = ml.alpha_decay?.half_life_days;
    return v != null ? `${Number(v).toFixed(1)}d` : '—';
  }
  if (objective === 'oos_is_ratio') {
    const isS = ml.is_vs_oos?.is_sharpe;
    const oosS = ml.is_vs_oos?.oos_sharpe;
    if (isS != null && oosS != null && Math.abs(Number(isS)) > 1e-9) {
      return (Number(oosS) / Number(isS)).toFixed(2);
    }
    return '—';
  }
  if (objective === 'robust_score') {
    const sharpe = summary.sharpe_ratio;
    const trades = row.trade_count ?? summary.total_trades ?? 0;
    if (sharpe == null) return '—';
    return (Number(sharpe) * Math.sqrt(Math.min(Number(trades) || 0, 100))).toFixed(2);
  }
  if (row.error) return '—';
  return `$${Number(row.total_pnl ?? 0).toFixed(2)}`;
}

export default function TaOptimizerPanel({
  symbol,
  strategy,
  days,
  timeframe,
  ...
  panelTitle = 'Parameter optimizer',
  panelSubtitle,
  hideHero = false,
  footerSlot = null,
  deploySlot = null,
  ...
  const [objective, setObjective] = useState(() => getDefaultObjective(strategy));
  const [minTrades, setMinTrades] = useState(() => getDefaultMinTrades(strategy));
  ...
  const applyConfig = (cfg) => {
    if (!cfg) return;
    const extras = typeof getDeployExtras === 'function' ? getDeployExtras() : null;
    replaceBotConfig(buildAppliedDeployConfig(strategy, cfg, {
      current: botConfig,
      extras,
    }));
  ...
  const deployOptimized = () => {
    ...
    const extras = typeof getDeployExtras === 'function' ? getDeployExtras() : null;
    // Set strategy before config so any strategy-tied UI sees the target id.
    ...
    <div className="algo-backtest-sweep algo-backtest-sweep--lab">
      ...
      <header className="algo-backtest-sweep__hero">
        <div className="algo-backtest-sweep__hero-copy">
          <h4 className="algo-backtest-sweep__title">{panelTitle}</h4>
