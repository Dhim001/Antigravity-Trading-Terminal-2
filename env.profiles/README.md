...
- **Alpaca** instance uses real Alpaca OMS (paper/live via `ALPACA_BASE_URL`). Equities via SIP/IEX, crypto via `v1beta3/crypto/us` (terminal `BTCUSDT` → wire `BTC/USD`; watchlist limited to Alpaca US bases), options via `v1beta1/indicative|opra` (streamed for charts/OMS — OCC contracts are **not** listed in the Watchlist sidebar). WS connect sends a full quote snapshot so late joiners / weekend equities are not stuck at `…`. Only one process may hold the SIP equity stream per Alpaca account.
- **Alpaca bots:** `ALLOW_LIVE_BOTS=true` in `alpaca.env` routes orders through Alpaca paper/live OMS (not simulated fills).
